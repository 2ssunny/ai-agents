...
`commit.author`를 자기 것으로 바꾼다. 이 값이 커밋 `--author` 서명에 쓰이고, 안 채우면 에이전트가 커밋 전에 물어본다. 브랜치 전략(`prBaseBranch`, `protectedBranches`)과 답변 언어·말투(`communication`)도 여기서 정한다.

이 파일은 `.gitignore`에 있어서 저장소에 올라가지 않는다. 덕분에 저장소에는 특정 개인의 정체성이 남지 않는다. **시크릿은 넣지 않는다** — 에이전트가 읽어도 되는 설정 파일이고, `.env`·`config.json` 같은 시크릿 파일은 여전히 열람 금지다.

### 3단계 — 전역 규칙 연결 (포인터 파일 만들기)
...
가장 자주 하는 작업이다. 전역 스킬(4단계)은 이미 어디서나 쓸 수 있지만, **Codex·Antigravity에 스킬을 노출하거나** 프로젝트별 스킬을 붙이려면 프로젝트마다 링크가 필요하다.
...
| `release` | "릴리스 하자", "버전 올려", CI/CD 워크플로우 셋업·디버깅 |
...
`release`는 두 개의 게이트를 강제한다 — **버전 레벨은 사용자가 고르고**, **태그 대상 커밋에서 CI가 통과해야** 태그·릴리스가 진행된다. CI/CD 파이프라인을 새로 짜거나 고칠 때의 기본 형태(와 프로젝트 규모에 따라 완화하는 기준)는 [`references/ci-cd-patterns.md`](skills/global/release/references/ci-cd-patterns.md)에 있다.
...
│   │   ├── release/              버전 태그·릴리스 + CI/CD 패턴 (+ references/)
│   │   ├── sync-docs/            문서·노션 동기화
